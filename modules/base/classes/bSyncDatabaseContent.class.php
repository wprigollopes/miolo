...
class bSyncDatabaseContent
{
    ...
    public function syncronize()
    {
        ...
        $ok = bBaseDeDados::consultar( $this->mountSqlCount(), $this->module );
        $result->countStart = $ok[0][0];
        $result->countXml = count($items);
        // Se tiver dados e possui a flag runOnlyEmpty a tabela não é alterada.
        if ( !($result->countStart > 0 && $this->xml->runOnlyEmpty == 't') )
        {
            //faz atualizações
            if ( count( $items ) > 0 )
            {
                foreach ( $items as $line => $item )
                {
                    ...
                    if ( is_array( $ok ) )
                    {
                        ...
                        {
                            //Verifica se o registro é igual pra não fazer um update desnecessario
                            if ( !$this->registerIsEgual( $item ) )
                            {
                                $sql= $this->mountUpdateSql( $item );
                                $ok = bBaseDeDados::consultar( $sql, $this->module );
        ...
        if ( count( $deletes ) > 0 )
        {
            foreach ( $deletes as $line => $delete )
            {
                ...
                if ( is_array( $ok ) )
                {
                    $ok = bBaseDeDados::consultar( $this->mountDeleteSql($delete), $this->module );
                    $deleteCount++;
                }
            }
        }
        ...
        if ( $this->compare )
        {
            ...
            if ( $result->countEnd != $result->countXml )
            {
                ...
                $all = bBaseDeDados::consultar( $sqlListAll, $this->module );
                $columns = bCatalogo::listarColunasDaTabela( $this->table, $this->module );
    ...
    public function locateItem( $item )
    {
        ...
        return bBaseDeDados::consultar( $sqlLocate, $this->module );
    }
    ...
    /**
     * Verifica se o registro é igual ao da base
     * 
     * @param SimpleXmlElement $item
     * @return boolean
     */
    public function registerIsEgual( $item )
    {
        $dataString = array();
        ...
        if ( is_array( $fields ) )
        {
            ...
            $checkWhereString = implode(' AND ', $dataString);
            $checkSql = "SELECT count(*) > 0 FROM {$this->table} WHERE $checkWhereString";
            $check = bBaseDeDados::consultar( $checkSql, $this->module );
    ...
    {
        $column = implode(', ', bCatalogo::listarColunasDaTabela($this->table, $this->module) );
