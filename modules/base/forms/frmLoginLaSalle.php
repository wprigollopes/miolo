...
class frmLoginLaSalle extends MForm
{
    ...
    public function createFields()
    {
       ...
       ');
        ...
        if ( !$this->isAuthenticated() )
        {
            ...
            $btnLogin->addBoxStyle('color', 'white');
            $btnLogin->addBoxStyle('box-shadow', '3px 2px 2px #999');
            $btnLogin->addBoxStyle('background-image', 'none');
    ...
    private function getUserField()
    {
        ...
        $labelEstrangeiros = new MSpan("", _M("Aluno estrangeiro, digite o número da Matricula"), "hint");
        return new MDiv("divContainerCampoUsuario", array($label, $uid, $labelEstrangeiros));
    ...
    private function obterCampoSenha()
    {
        ...
        $pwd = new MTextField("pwd", "", _M("Data de nascimento"), 20);
        $pwd->setAttribute("placeholder", _M("Data de nascimento"));
        $pwd->addAttribute('onKeyPress', 'MascaraData(this)');
        $pwd->addAttribute('onBlur', 'ValidaData(this)');
        $pwd->addAttribute('maxlength', '10');
    ...
    public function btnLogin_click()
    {
        ...
        $MIOLO->uses('/security/mAuthMIOLO.php');        
	...
	$MIOLO->getSession()->setValue('pwd', $pwd);
        ...
        if ( SAGU::authenticate($user, $pwd) ||  SAGU::authenticate($user, str_replace("/", "", $pwd)) )
        {
        ...
        else
        {
            ...
            if ( $this->auth->errors )
            {
                $err = new MExpandDiv(NULL, _M('Usuário e/ou senha incorretos.') . '<br/>' . $this->auth->errors);
            }
            else
            {
                $err = _M('Usuário e/ou senha incorretos.');
            }
