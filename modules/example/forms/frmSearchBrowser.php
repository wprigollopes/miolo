...
class frmSearchBrowser extends MForm
{
    ...
    {
        parent::__construct(_M('Browser Search', MIOLO::getCurrentModule()));
        $this->eventHandler();
    }
    ...
    public function createFields()
    {
        ...
        $fields[] = new MTextField('identifier', NULL, _M('Id', $module), 10);
        $fields[] = new MTextField('description', NULL, _M('Description', $module), 50);

        $searchButtons[] = new MButton('clearButton', _M('Clear'));
        $searchButtons[] = new MButton('searchButton', _M('Search'));
     ...
     */
    public function actionDelete_click($args)
    {
        $MIOLO = MIOLO::getInstance();
        $module = MIOLO::getCurrentModule();
        $browser = $MIOLO->getBusiness($module, 'browser');
        $browser->setIdentifier(MIOLO::_REQUEST('item'));

        if ( $browser->delete() )
        {
            $this->grid->setData($browser->search($data));
            $this->setResponse($this->grid->generate(), 'divGrid');
            new MMessageSuccess(_M('Register removed successfully!'));
        }
    }
}
