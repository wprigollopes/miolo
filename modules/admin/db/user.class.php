...
class BusinessAdminUser extends MBusiness implements IUser
{
    ...
    public function insert()
    {
        $MIOLO = MIOLO::getInstance();
        $db = $MIOLO->getDatabase( 'admin' );
        $this->hash = self::getHash( $this->password );
    ...
    {
        if ( BusinessAdminUser::getHash($password) != $this->password )
        {
    ...
    {
        $MIOLO = MIOLO::getInstance();
        $method = $MIOLO->getConf('login.method');

        if( strlen($method) == 0 )
        {
            $method = 'md5'; //Caso não definido o método no miolo.conf mantém o md5 como padrão
        }
