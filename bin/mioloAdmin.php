...
class MioloAdmin
{
    ...
    public function start($mioloPath)
    {
        ...
        symlink("$svnPath/bin/mioloAdmin.php", "$mioloPath/bin/mioloAdmin.php");
        ...
        symlink("$svnPath/modules/modules.php", "$mioloPath/modules/modules.php");
    ...
    public function createModule($module, $baseModule=NULL)
    {
        ...
        // Create handler.php
        $template = $this->readTemplateContent('handlerTemplate.php');
        $content = str_replace('#Module', ucfirst($module), $template);
        $this->writeContentToFile($content, "$moduleDir/handlers/handler.php");
    ...
    public function createSampleModule($module)
    {
        ...
        $template = $this->readTemplateContent('mainTemplate.php');
    ...
    public function copySamplesToModule($module, $db=NULL)
    {
        ...
        $this->copySampleToModule('browserSample.php', $businessDir, $module, $db);
        $this->copySampleToModule('lookupSample.php', $businessDir, $module, $db);
        $this->copySampleToModule('browserSampleInc.php', 'handlers', $module, $db);
        $this->copySampleToModule('lookupSampleInc.php', 'handlers', $module, $db);
        $this->copySampleToModule('frmBrowserSample.php', 'forms', $module, $db);
        $this->copySampleToModule('frmSearchBrowserSample.php', 'forms', $module, $db);
        $this->copySampleToModule('grdBrowserSample.php', 'grids', $module, $db);
    ...
    public function createHandler($module, $handler, $title, $form)
    {
        ...
        $template = $this->readTemplateContent('handlerTemplateInc.php');
        $content = str_replace('#title', $title, $template);
        $content = str_replace('#formName', $form, $content);

        $filePath = "$moduleDir/handlers/$handler.php";
    ...
    public function createForm($module, $form, $title, $fields = '')
    {
        ...
        $template = $this->readTemplateContent('frmFormTemplate.php');
        $content = str_replace('#title', $title, $template);
        $content = str_replace('#Form', $Form, $content);
        $content = str_replace('#fields', $fields, $content);

        $filePath = "$moduleDir/forms/frm$Form.php";
        $this->writeContentToFile($content, $filePath);
        return $filePath;
    }
    ...
    public function createSearchForm($module, $form, $title, $grid, $table, $filters = array())
    {
        ...
        $template = $this->readTemplateContent('frmSearchFormTemplate.php');
        $content = str_replace('#title', $title, $template);
        $content = str_replace('#Form', $Form, $content);
    ...
    public function createGrid($module, $grid, $title, $columns = array())
    {
        ...
        $template = $this->readTemplateContent('grdGridTemplate.php');
        $content = str_replace('#title', $title, $template);
        $content = str_replace('#Grid', $Grid, $content);
        ...
        $filePath = "$moduleDir/grids/grd$Grid.php";
        $this->writeContentToFile($content, $filePath);
        return $filePath;
    }
    ...
    public function createBusiness($module, $table, $filters = array())
    {
        ...
        $template = $this->readTemplateContent('tableTemplate.php');
        ...
        $filePath = "$moduleDir/db/$table.php";
        $this->writeContentToFile($content, $filePath);
        return $filePath;
    }
    ...
    public function createTheme($theme, $themeBase)
    {
        ...
        foreach ( scandir($themeBaseDir) as $file )
        {
            ...
            elseif ( preg_match('/^theme.*\.php$/', $file) )
            {
                ...
                $newFile = 'theme' . ucfirst($theme) . '.php';
                $this->writeContentToFile($themeClass, "$themeDir/$newFile");
            }
            else
            {
