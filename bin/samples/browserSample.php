...
class Business#ModuleBrowser extends MBusiness
{
    ...
    public function update()
    {
        ...
        $msql->setWhere("identifier = ?");
        $sql = $msql->update(array( $this->description, $this->identifier ));
        return $this->getDb()->execute($sql);
    }
    ...
    public function delete()
    {
        ...
        $msql->setWhere("identifier = ?");
        $sql = $msql->delete($this->identifier);
        return $this->getDb()->execute($sql);
    }
     ...
     * @return MQuery Search result object
     */
    public function getSearch($filters=NULL)
    {
        $parameters = array();
        ...
        {
            $msql->setWhere("identifier = ?");
            $parameters[] = $filters->identifier;
        }

        if ( strlen($filters->description) )
        {
            $msql->setWhere("lower(description) LIKE ?");
            $parameters[] = strtolower($filters->description);
        }

        $msql->setParameters($parameters);
        return $this->getDb()->getQuery($msql);
    }
