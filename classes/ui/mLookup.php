...
class MLookup
{
    ...
    public function __construct($baseModule = 'admin')
    {
        ...
        $this->filterValue  = isset($_GET['filter']) ? $_GET['filter'] : null;
        $this->type         = isset($_GET['type']) ? $_GET['type'] : null;
        $this->title        = isset($_GET['title']) ? $_GET['title'] : null;
        $this->autocomplete = isset($_GET['autocomplete']) ? $_GET['autocomplete'] : null;
        $params = array(
...
//        $this->grid->setId($this->formName);
    ...
    public function execute()
    {
        ...
        $fileName = $MIOLO->getConf('namespace.business').'/lookup' . $MIOLO->php;
