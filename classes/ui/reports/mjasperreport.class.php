...
require __DIR__ . "/../../../../miolo20/modules/basic/classes/phpjasper/vendor/autoload.php";
...
class MJasperReport extends MReport
{
     ...
     * @throws Exception
     */
    public function generatePhpJasper($fileIn, $fileOut, $filetype, $parameters, $classPath, $download = true)
    {
        $MIOLO = MIOLO::getInstance();
        ...
        try
        {
            $parametros = $jasper->listParameters($input)->execute();

            foreach ($parametros as $parametro)
            {
                $parametroRelatorio[] = substr(substr($parametro,0, strpos($parametro, ' ',2)),2, strpos($parametro, ' ',2));
            }

            foreach ($parameters as $cod=>$valor)
            {
                if( in_array(str_replace('str_', '', str_replace('int_', '', $cod)), $parametroRelatorio) )
                {
                    $parametrosNovo[str_replace('str_', '', str_replace('int_', '', $cod))] = $valor;
                }
            }

            if( strstr($dbHost, ':') )
            {
                $explode = explode(':',$dbHost);
                $newDbHost = $explode[0];
                if( strlen($newDbHost) > 0 )
                {
                    $dbHost = $newDbHost;
                }
            }
            ...
            $options = [
                ...
                'resources' => dirname($input)
            ];

            $jasper->process($input, substr($output, 0, strlen($output)-strlen(pathinfo($output, PATHINFO_EXTENSION))-1),$options)->execute();
        }
        catch(Exception $e)
        {
            $detalhes = _M("Ocorreu um erro durante a geração do relatório.");
            $detalhes .= "<br/>Detalhes do erro:<br/> ";
            $detalhes .= $e->getMessage();
            $detalhes .= trim(implode("\n",$output));
    ...
    public function fill($fileIn, $fileOut, $filetype, $parameters, $classPath, $save, $download = true)
    {
        $this->beforeGenerateReport();

        $MIOLO = MIOLO::getInstance();

        $generateReport = $this->generatePhpJasper($fileIn, $fileOut, $filetype, $parameters, $classPath, $download);
        ...
        if (  $generateReport )
        {
            ...
            $this->setArquivoRecemGerado($fileOut);
     ...
     * @return integer Returns 1 if succeeded.
     */
    public function executeJRXML($module, $name, $parameters=NULL, $filetype='PDF', $save=FALSE, $download = false)
    {
        $MIOLO = MIOLO::getInstance();
        ...
        $pathLibs = "$pathMJasper/lib/deps/*:$pathMJasper/lib/ext/*:$pathMJasper/lib/ireport/*:$pathMJasper/lib/jasperreports/*";
        $classPath = "$pathLibs:$pathMJasper/:$pathJava/lib";
        $fill = $this->fill($fileIn, $pathout, $this->filetype, $parameters, $classPath, $save, $download);
     ...
     */
    public static function getParameters($module = null, $report = null)
    {
        $MIOLO = MIOLO::getInstance();
    ...
    public function setArquivoRecemGerado($arquivoRecemGerado)
    {
        $this->arquivoRecemGerado = $arquivoRecemGerado;
    }
