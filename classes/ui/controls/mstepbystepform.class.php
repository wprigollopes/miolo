...
class MStepByStepForm extends MForm
{
    ...
    public function getStepData($step = null, $returnAsObject = true)
    {
        ...
        if ( $this->isLastStep() )
        {
            ...
            $stepsIds = array_keys($this->steps);
            $lastStep = array_pop($stepsIds);
        ...
        if ( $step )
        {
            $stepData = isset($stepsData[$step]) ? $stepsData[$step] : null;
            $data = $returnAsObject ? (object) $stepData : $stepData;
        }
        else
        {
            if ( $returnAsObject )
     ...
     *
     */
    public function cancelButton_click()
    {
    ...
    public function finalizeButton()
    {
        ...
        return new MButton(self::FINALIZE_BUTTON_ID, _M('Finalize'), ':finalizeButton_click', $image);
    }
