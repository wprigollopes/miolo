...
class MPrompt extends MInputControl
{
    ...
    public function setType( $type = MPrompt::MSG_TYPE_INFORMATION )
    {
        ...
        {
            case 'error':
                $this->panelCssClass = 'panel-danger';
                break;
            case 'information':
                $this->panelCssClass = 'panel-info';
                break;
            case 'alert' || 'question':
                $this->panelCssClass = 'panel-warning';
                break;
            case 'confirmation':
                $this->panelCssClass = 'panel-success';
                break;
            default:
                $this->panelCssClass = 'panel-info';
                break;
    ...
    public function generateInner()
    {
        ...
        $c->setClass("mPromptBoxBody mPromptBox{$type} panel-body");
        $this->inner = $c;
    }
    ...
    function generate()
    {
        $this->generateInner();
        $type = ucfirst($this->type);
        $this->box = new MBox($this->caption, $this->close, '', $this->panelCssClass);
        $this->box->boxTitle->setClass("mPromptBoxTitle mPromptBoxTitle{$type} panel-heading");
