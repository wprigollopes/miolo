...
class MCaptchaField extends MContainer
{
    public function __construct( $name, $label = '', $hint = '', $size = 10 )
    {
        ...
        $controls[] = new MImage("{$name}Image", NULL, $url[0] . "/extensions/securimage/securimage_show.php?sid=$uniqueId");

        $controls['button'] = new MButton("{$name}Button", ' ', self::getRefreshCode($name));
     ...
     */
    public static function getRefreshCode($name)
    {
        $MIOLO = MIOLO::getInstance();
        $url = $MIOLO->getCurrentURL();
        $url = explode('index.php', $url);
        
        $code = "captchaImage = dojo.byId('{$name}Image');";
        $code .= "if (captchaImage) { captchaImage.src = '{$url[0]}/extensions/securimage/securimage_show.php?sid=' + Math.random() };";
