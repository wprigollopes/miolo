...
class MButton extends MActionControl
{
    ...
    public function __construct( $name = '', $label = '', $action = NULL, $image = NULL, $onclickdisable = FALSE )
    {
        ...
        $this->setClass('mButton btn btn-default');
        ...
        if ( $this->manager->checkMobile() )
        {
            ...
            // FIXME: Added to avoid duplicated posts. Probably a Dojo bug.
            $this->page->onload("if ( dijit.byId('$this->id') ) dijit.byId('$this->id').domNode.setAttribute('onclick', '');");
        }
    }
