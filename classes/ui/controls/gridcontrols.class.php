...
class MGridAction extends MControl
{
    ...
    public function generateLink($row)
    {
        ...
        return (($this->grid->linktype == 'hyperlink' && $this->grid->eventGo !== false) ? 'go:' : '') . $href;
    }
...
class MGridActionIcon extends MGridAction
{
    ...
    protected $staticHref;

    public function __construct($grid, $value, $href, $alt = null, $staticHref = null)
    {
    ...
    public function generate()
    {
        ...
        if ($this->enabled)
        {
            ...
            $href = $this->staticHref ? "javascript:void(0);" : $this->generateLink($row);
            $img = $this->grid->getImage($this->value);
            $control = new MImageButton('', $this->alt, $href, $path);

            if ( !empty($this->staticHref) )
            {
                $control->setOnClick($this->staticHref);
            }
        }
        else
        {
...
class MGrid extends MBaseGrid
{
    ...
    public $actions = array(); // array with actions controls
    ...
    private $gridMostraTotalNumeric = false;
...
*/    
     ...
     */
    public function __construct($data, $columns, $href=NULL, $pageLength=15, $index=0, $name='', $useCheckBoxes=TRUE, $gridMostraTotalNumeric=false)
    {
        ...
        $this->gridMostraTotalNumeric = $gridMostraTotalNumeric;
     ...
     * @return boolean
     */
    public function isGridMostraTotalNumeric()
    {
        return $this->gridMostraTotalNumeric;
    }

    /**
     * @param boolean $gridMostraTotalNumeric
     */
    public function setGridMostraTotalNumeric($gridMostraTotalNumeric)
    {
        $this->gridMostraTotalNumeric = $gridMostraTotalNumeric;
    }
    ...
    public function addActionIcon($alt, $icon, $href, $index = 0, $staticHref = null)
    {
        if ($p = strpos($icon,'.')) $icon = substr($icon,0,$p);
        $this->actions[] = new MGridActionIcon($this, $icon, $href, $alt, $staticHref);
    }
...
//        $this->addActionIcon('Excluir', array('button_drop.png', 'button_noempty.png'), $href);
    ...
    public function handlerSelecteds()
    {
        ...
        if ($useSelecteds)
        {
            ...
            if (isset($selecteds[$this->pageNumber]) && is_array($selecteds[$this->pageNumber]))
            {
    ...
    public function generateColumnsHeading(&$tbl)
    {
        ...
        if ($this->select != NULL)
        {
            ...
            if ( $this->gridMostraTotalNumeric )
            {
                $rowCount --;
            }
    ...
    public function generateActions(&$tbl)
    {
        ...
        if ( $this->gridMostraTotalNumeric && ($this->pageLength <= $i || $i == $this->rowCount) )
        {
            $tbl->setRowAttribute($i, 'id', "total_grid");
            $label = "<span class='m-label'><b>TOTAL DA PÁGINA</b></span>";
            $tbl->setCell($i, $c, $label);
            $tbl->setCellClass($i, $c, 'data action');
            $tbl->setCellAttribute($i, $c, "colspan", $n);
            $c++;

            if ( $this->select != null )
            {
                $hidden = new MHiddenField("select" . $this->name . "[{$this->currentRow}]");
                $tbl->setCell($i, $c, $hidden->generate());
...
//                $tbl->setCellAttribute($i,$p,"width",$col->width);
    ...
    public function generateBody()
    {
        ...
        $tbodyId = 'tbody' . $this->id;
        
        $tblData = new MSimpleTable($tbodyId, "cellspacing=\"0\" cellpadding=\"0\" border=\"0\" width=\"100%\" class=\"mGridBody\"");
        
        $this->page->onLoad( "if (typeof $('#{$tbodyId}').stickyTableHeaders == 'function' ) $('#{$tbodyId}').stickyTableHeaders();" );
        ...
        if ($this->data)
        {
            ...
            foreach ($this->data as $row) // foreach row
            {
                ...
                if ( strlen($this->getRowClickMethod()) > 0 )
                {
                    $tblData->setRowAttribute($i-1, 'onclick', MUtil::getAjaxAction($this->getRowClickMethod(), $row[0]));
                }
            } // end foreach row
        }// end if
    ...
    {
        $url = isset($this->icons[$src]) ? $this->icons[$src] : null;
        if (!$url)
        {
    ...
    public function setQuery($sql, $dbconf)
    {
        ...
        $sqlComSomatorios = $sqlWithLimit;

        if ( $this->gridMostraTotalNumeric && $this->rowCount > 0 )
        {
            $sqlComSomatorios = $this->obterConsultaComSomatorios($sqlWithLimit);
        }

        try
        {
            $query = $db->query($sqlComSomatorios);
        }
        catch ( Exception $err )
        {
        ...
        $this->data = is_array($query) ? $query : (array) $query->result;
     ...
     * @return string
     */
    public static function obterConsultaComSomatorios($query)
    {
        $randConsulta = rand();
        $sql = "
            CREATE TEMP TABLE consulta_{$randConsulta} ON COMMIT DROP AS (
                {$query}
            );
            SELECT *
              FROM consulta_{$randConsulta}
         UNION ALL   
            SELECT * 
              FROM somatorioDeValoresDaTabela(NULL::consulta_{$randConsulta})
        ";
     ...
     * @return mixed
     */
    public static function obterConsultaComNomesDasColunas($query)
    {
        $randConsulta = rand();
        $sql = "
            CREATE TEMP TABLE consulta_{$randConsulta} ON COMMIT DROP AS (
            ...
            SELECT column_name
	         ...
	         WHERE table_name = 'consulta_{$randConsulta}'
        ";
     ...
     * @return string
     */
    public static function normalizaConsulta($sql, $dbconf, $gridMostraTotalNumeric = false)
    {
        if ( $gridMostraTotalNumeric )
        {
            $MIOLO = MIOLO::getInstance();
            $db = $MIOLO->getDatabase($dbconf);
            ...
            foreach ($msql->getColumns() as $key => $column)
            {
                ...
                $columns[$key . '_' . $count] = $column;
                $explodeColumn = explode(" as ", strtolower($column));
                if (count($explodeColumn) < 2 && $column != '*')
                {
                    $columns[$key . '_' . $count] = $column . " AS coluna_" . $count;
                }
    ...
    public function getRowClickMethod()
    {
        return $this->rowClickMethod;
    }

    public function setRowClickMethod($rowClickMethod)
    {
