...
class MHtmlPainter extends MBasePainter
{
    ...
    public function htmlelement( $control )
    {
	return $control->getInnerToString();
    }
    ...
    public function table( $control )
    {
        ...
        if ($n = count((array) $control->colgroup))
        {
            for($i=0; $i<$n; $i++)
            {
                $html .= "\n<colgroup " . $control->colgroup[$i]['attr'] . ">" ;
                $k = isset($control->colgroup[$i]['col']) ? count($control->colgroup[$i]['col']) : 0;
                for($j=0; $j<$k; $j++)
                {
        ...
        {
            $html .= "\n<thead><tr class=\"m-grid-row-heading\">";
            for($i=0; $i<$n; $i++)
            {
                $attributes = isset($control->attr['head'][$i]) ? $control->attr['head'][$i] : '';
                $html .= "\n<th " . $attributes . ">" . $control->head[$i] . "</th>";
            }
            $html .= "\n</tr></thead>";
        }
