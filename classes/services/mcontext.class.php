...
class MContext extends MService
{
    ...
    public function parseUrl($url)
    {
        ...
        $this->host = '//' . $urlParts['host'] . (isset($urlParts['port']) ? ':' . $urlParts['port'] : '');
    ...
    private function parseURI()
    {
        ...
        {
            $regexp = "~/({$this->dispatch})(\?)(?P<module>module=[^&]*)?(&?action=(?P<action>[^&.]*)(&|\.|$)(.*)|$)~";
        }
        else
        {
            $regexp = "~/({$this->dispatch})(/?)(?P<module>[^/]*)(/(?P<action>[^&.]*)(&|\.|$)(.*)|$)~";
        }
        ...
        if ( preg_match($regexp, $uri, $parts) !== FALSE )
        {
            ...
            $this->fileType = isset($parts[self::TYPE]) ? $parts[self::TYPE] : null;
            $this->getTokens();
            $this->item = isset($this->vars['item']) ? $this->vars['item'] : null;
            $this->forceStartup = isset($this->vars['_startup']) ? $this->vars['_startup'] : null;
        }
        else
        {
