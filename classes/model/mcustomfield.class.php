...
class MCustomField26
{
    ...
    public function setData($data)
    {
        ...
        $this->filtroFormularioBusca = $data->filtroFormularioBusca;
        $this->visivel = $data->visivel;
    }
    ...
    public function insert()
    {
        ...
        $sql = new MSQL(
            'id, identifier, name, field_format, possible_values, regexp, min_length, max_length, required, position, default_value, editable, visible, filtroFormularioBusca, visivel',
            'miolo_custom_field'
        );
    ...
    {
        $sql = new MSQL(
            'identifier, name, field_format, possible_values, regexp, min_length, max_length, required, position, default_value, editable, visible, filtroFormularioBusca, visivel',
            'miolo_custom_field',
            'id = ?'
        );
    ...
    {
        $sql = new MSQL(
            'identifier, name, label, field_format, possible_values, regexp, min_length, max_length, required, position, default_value, editable, visible, possible_parameters, filtroFormularioBusca, visivel',
            'miolo_custom_field',
            'id = ?'
        );
    ...
    {
        return MUtil::getBooleanValue($this->visivel);
    }
    ...
    {
        $sql = new MSQL(
            'id, identifier, name, label, field_format, possible_values, regexp, min_length, max_length, required, position, default_value, editable, visible, possible_parameters, filtroFormularioBusca, visivel',
            'miolo_custom_field',
            'identifier ilike ?',
            'position, name'
        );
    ...
    public function getListSQL()
    {
        ...
        if ( strlen($this->possibleValues) > 0 && ( $this->fieldFormat == MCustomField26::FORMAT_LISTSQL ) )
        {
    ...
    public static function getFieldByTableColumn($table, $column)
    {
        $msql = new MSQL('id');
        $msql->setTables('miolo_custom_field');
        $msql->setWhere('tabela = ? AND coluna = ?', array($table, $column));
        $query = self::getBusiness()->query($msql);

        if (strlen($query->result[0][0]) > 0)
        {
            $retorno = new MCustomField26();
            $retorno->id = $query->result[0][0];
            $retorno->populate();
            return $retorno;
        }
