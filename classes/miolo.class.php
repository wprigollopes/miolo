	...
	if ( !isset($GLOBALS['MIOLO_UPDATER']) && file_exists($this->getConf('home.miolo') . '/.down') )
        ...
        else
        {
            ...
            catch( EMioloException $e )
            {
                ...
                {
                    $msg = $this->getConf('options.message.error');
                }
...
//$this->getObject('trace')->tracestack();
	    ...
	    $extension = (strstr($path,'modules') === FALSE) ? $extension : ($m->getConf("options.fileextension") == '2' ? '' : '.php');
            $path .= (empty($pathinfo['extension']) ? '.class'.$extension : '');
...
//                echo "MIOLO: $errmsg";
    ...
    public function init( $home = NULL, $logname = 'miolo')
    {
        ...
        $this->setConf('options.message.error', 'O sistema não consegue acessar o banco de dados. Este problema pode ser ocasionado por instabilidade na rede local, servidor do banco de dados parado, entre outros. <b>Favor contatar o suporte de infraestrutura</b>.');
    }
...
//          $msg .= $e->getFile() . $e->getLine() . $e->getTraceAsString();
    ...
    public function getHandler( $module )
    {
        ...
        $this->handlers[$class] = isset($this->handlers[$class]) ? $this->handlers[$class] : null;
    ...
    /**
     * Removido addslashes a partir do PHP 7.0. Ticket #87548.
     */
