...
class MDatabase
{
    ...
    public function __construct($conf, $system, $host, $db, $user, $pass, $persistent, $parameters=NULL, $server=false, $jdbc_driver=NULL, $jdbc_db=NULL, $altPort = null)

    {
        ...
        $this->_miolo->uses('database/mConnection.php');
        $this->_miolo->uses('database/mDataSet.php');
        $this->_miolo->uses('database/mQuery.php');
        $this->_miolo->uses('database/mIdGenerator.php');
        $this->_miolo->uses('database/mTransaction.php');
        $this->_miolo->uses('database/mSqlJoin.php');
        $this->_miolo->uses('database/mSchema.php');

        $this->_miolo->uses('database/' . $system . '/mConnection.php');
   ...
   {
        $this->conn->open($this->host, $this->db, $this->user, $this->pass, $this->persistent, $this->parameters, $this->port);
    ...
    {
        $this->_miolo->uses('database/' . $this->system . '/mTransaction.php');
    ...
    {
        $this->_miolo->uses('database/mISR.php');
    ...
    {
        $this->_miolo->uses('database/' . $this->system . '/mIdGenerator.php');
    ...
    public function query($sql, $maxrows=0, $offset=NULL, $fetchType=NULL)
    {
        ...
        if($this->_miolo->getConf('options.miolo2modules')) //miolo2 modules integration
        {
            return $query->result;
        }
    ...
    public function objQuery($sql = null, $maxrows = 0) // backward compatibility
    {
        ...
        return $this->getQuery($msql, $maxrows);
    }
    ...
    {
        $this->_miolo->uses('database/' . $this->system . '/mSchema.php');
