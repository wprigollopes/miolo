...
class MIOLO
{
    ...
    public function getNamespacePath($namespace, $miolo2integration=false)
    {
        $m = MIOLO::getInstance();
        
        if($miolo2integration)
        {
            $path = $m->getConf('options.miolo2modules');
        }
        else
        {
    ...
    public function import($namespace, $class = '', $extension = '.php', $miolo2integration=false)
    {
        $m = MIOLO::getInstance();
...
//$this->getObject('trace')->tracestack();
        ...
        {   
            $path = $m->getNamespacePath($namespace, $miolo2integration);
            ...
            elseif (!isset($pathinfo['extension']))
            {
                ...
                $path .= $extension;
                // Fallback to legacy .class extension if new path not found
                if (!file_exists($path)) {
                    $legacyPath = str_replace($extension, '.class' . $extension, $path);
            ...
            if(!file_exists($path))
            {
                $path = str_replace($m->getConf('home.miolo'), $m->getConf('options.miolo2modules'), $path);
            }
            
            if(!file_exists($path))
            {
                $path = str_replace('.class.php', '.class', $path);
            }
            
            $path = str_replace('/modules/modules/', '/modules/', $path);
...
//                echo "MIOLO: $errmsg";
    ...
    public function init( $home = NULL, $logname = 'miolo')
    {
        ...
        /**
         * Ticket #46247
         * The getConf of modules.php was moved further down in this function because
         * we were having issues using MUtil functions (which was not yet instantiated) in modules.php.
         */
        
        // get the modules.inf
        require_once $this->getConf('home.modules') . '/modules.php';
    }
...
mdump($this->getConf("options.fileextension"));
    ...
    public function handler()
    {
        ...
        try
        {
            ...
            if ($this->checkLogin())
            {
                if (!$this->isAjaxCall && MIOLO::_REQUEST('disableAjax') != '1' )
                {
...
//          $msg .= $e->getFile() . $e->getLine() . $e->getTraceAsString();
    ...
    public function getHandler( $module )
    {
        ...
        $handler = isset($this->handlers[$class]) ? $this->handlers[$class] : null;
     ...
     * ...
     * </code>
     * O exemplo acima retorna um link para o handler list_person.php, no
     ...
     * @param $action (string) Nome do handler (<code>.php</code>) a ser acessado
    ...
    public function saguCompatibility($url)
    {
        ...
        {
            if(strstr($url,'/portal/')) //devido a instalacao
            {
                $url = str_replace('/portal/', '/miolo20/', $url);
            }
            else
            {
    ...
    public function getModulePath($module, $file)
    {
        ...
        if(!file_exists($path))
        {
            $path = $this->getConf('options.miolo2modules').'/'.$module.'/'.$file;
            
            if(!file_exists($path))
            {
                if(substr($file, 0, 3)=='Bus')
                {
                    $path = $this->getConf('options.miolo2modules').'/'.$module.'/db/'.$file;
                }
            }
        }
    ...
    public static function _REQUEST( $vars, $from = 'ALL', $order='' )
    {
        ...
        else
        {
            ...
            if(isset($_COOKIE[$vars]) && !isset($value))
            {
                $value = $_COOKIE[$vars];
            }
    ...
    #
    # @see #MIOLO::getBusiness, miolo/mDatabase.php
    #---------------------------------------------------------------------
    /**
     *
     ...
     *
     * @example
     * <i> in file: ../modules/foo/db/guestbook.php </i>
     * &lt;?
     *
     ...
     * ?&gt;
     * <hr>
     * <i> in file: ../modules/foo/handlers/register.php </i>
     * &lt;?
     *
    ...
    public function getBusiness($module, $name = 'main', $data = NULL)
    {
        ...
        $path = $this->getModulePath($module, 'types' . $this->php);

        if( file_exists($path) )
        {
            $this->uses('types' . $this->php, $module);
        }
    ...
    {
        $this->uses("/classes/$name.php", $module);
    }
    ...
    {
        $file = is_null($dir) ? "webservices/$name.php" : "webservices/$dir/$name.php";
     ...
     *
     * @param boolean $isMobile Whether is a mobile session.
     */
    public function setMobile($isMobile)
    {
        $session = $this->getSession();
        $session->setValue(self::IS_MOBILE_SESSION_ID, $isMobile);
    }

    /**
     * Check if MIOLO session is defined as mobile.
     *
     * @return boolean Whether is a mobile session.
     */
    public function checkMobile()
    {
        $session = $this->getSession();
        return $session->getValue(self::IS_MOBILE_SESSION_ID);
    }
