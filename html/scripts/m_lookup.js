...
//        miolo.getWindow(this.context.name).setTitle(this.context.title);
    ...
    deliver: function(name, key, args) {
        ...
        // Trigger the custom change event.
        var field = miolo.getElementById(context.field);
        var customChange = field.getAttribute('oncustomchange');

        // Adicionado para poder disparar um evento após o filler. 
        if ( customChange )
        {
            eval(customChange);
        }
